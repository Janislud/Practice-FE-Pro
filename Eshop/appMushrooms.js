...
const updatedMushroom = mushroom.map((product, index) => ({
    ...product,
    id: newId(),
}));
...
updateProducts(updatedMushroom); // Вызов функции для обновления продуктов на странице


updateProducts(updatedMushroom);

const sortButton = document.getElementById('sortButton')
sortButton.addEventListener('click',() => {
    updatedMushroom.sort((a, b) => a.price - b.price);
    updateProducts(updatedMushroom)
});

const sortButton2 = document.getElementById('sortButton2')
sortButton2.addEventListener('click',() => {
    updatedMushroom.sort((a, b) => b.price - a.price);
    updateProducts(updatedMushroom)
});
